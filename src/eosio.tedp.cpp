...
double tedp::getbalanceratio()
{
    ...
    auto evm_balance_string = intx::to_string(evm_balance, 10);
    auto evm_balance_intx = intx::from_string<uint256_t>(evm_balance_string);
    auto evm_balance_with_precision = evm_balance_intx / pow(10, 14);
    auto evm_balance_intx_string = intx::to_string(evm_balance_with_precision).c_str();
    const auto evm_total = atoi(evm_balance_intx_string); // Loosing precision but ratio won't be affected much
    const auto rex_total = (rex_pool.begin() != rex_pool.end()) ? rex_pool.begin()->total_lendable.amount : 0;
    print("rex_total: ", rex_total, "\n");
    print("evm_balance: ", intx::hex(evm_balance), "\n");
    print("evm_balance_string: ", evm_balance_string, "\n");
    print("evm_balance_intx: ", intx::hex(evm_balance_intx), "\n");
    print("evm_balance_with_precision: ", intx::hex(evm_balance_with_precision), "\n");
    print("evm_balance_intx_string: ", evm_balance_intx_string, "\n");
    print("evm_total: ", evm_total, "\n");
    print("fixed_ratio: ", fixed_ratio, "\n");
    auto retVal = (rex_total == 0) ? -1.0 : (evm_total * fixed_ratio) / double(rex_total);
    print("return value of getbalanceratio: ", retVal, "\n");
    return retVal;
}