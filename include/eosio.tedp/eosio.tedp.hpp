...
public:
    ...
    tedp(name receiver, name code, datastream<const char *> ds) : contract(receiver, code, ds), payouts(name("exrsrv.tf"), name("exrsrv.tf").value),  configuration(name("exrsrv.tf"), name("exrsrv.tf").value) {}
...
private:
   ...
   struct [[eosio::table,eosio::contract("eosio.system")]] rex_pool {
    ...
    using pay_action = action_wrapper<name("payout"), &tedp::testpay>;
